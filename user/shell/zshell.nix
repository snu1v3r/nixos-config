...
in
{
    ...
    sessionVariables = {
      MANPAGER = ( if ( config.programs.bat.enable == true ) then "sh -c 'col -bx | bat -l man -p'" else "less");
    ...
    plugins = [] ++
          ( if (userSettings.prompt == "p10k") then
              [
                {
                  name = "powerlevel10k";
  ...
  programs.starship.enableZshIntegration = ( if (userSettings.prompt == "starship") then true else false);
  programs.oh-my-posh.enable = ( if (userSettings.prompt == "oh-my-posh") then true else false);
  programs.oh-my-posh.enableZshIntegration = ( if (userSettings.prompt == "oh-my-posh") then true else false);
